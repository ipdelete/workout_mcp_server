...
def calculate_tss(duration_minutes: int, avg_power_watts: int, workout_type: str) -> int:
...
def generate_training_plan(start_date: date, num_days: int) -> List[str]:
    ...
    for week in range(num_days // 7 + 1):
        ...
        if week_number == 3:
            ...
            weekly_pattern = ["intensity", "recovery_week", "build", "intensity", "rest", "build", "rest"]
        else:
            weekly_pattern = ["build", "recovery_week", "build", "intensity", "rest", "build", "recovery_week"]
